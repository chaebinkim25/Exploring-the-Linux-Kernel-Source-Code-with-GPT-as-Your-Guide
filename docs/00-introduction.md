...
CPU와 메모리를 제외한 모든 전자 모듈 (모니터, 키보드, 마우스 등등)을 `장치`라고 부릅니다. 커널에 있어서의 장치는 실제 장치를 설명하는 정보들입니다. 그 정보들 중에는 드라이버라고 불리는 사용 설명서가 있어서 커널이 장치를 쓸 수 있게 해줍니다. 커널은 드라이버에게 장치에서 읽고 쓰기를 하거나 설정을 바꾸라고만 하고, 실제로 장치와 통신을 하는 것은 드라이버가 합니다. 드라이버가 장치와 통신을 할 때, 장치에 접근할 권한은 커널에게만 있기 때문에, 커널에게 요청에서 가상 메모리 주소를 받아서 장치에 데이터를 읽고 쓸 수 있습니다. 한편, 키보드 입력 등 장치에서 어떤 일이 발생했다고 CPU에 직접 전기 신호로 알릴 수 있습니다. 드라이버는 커널에 알림이 왔을 때 실행할 코드를 등록해두고, 커널은 알림이 오면 등록된 코드를 실행합니다.
