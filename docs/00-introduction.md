...
사용자에게 `프로세스`는 실행 중인 프로그램입니다. 반면, 커널에게 있어서 프로세스는 실행 중인 프로그램을 관리하기 위해서 ID, 상태, 할당받은 메모리, 열려 있는 파일 목록, CPU 쓸 때 우선 순위, 부모-자식 관계 등의 정보를 모아놓은 것입니다. `프로세스 관리`를 하는데 있어서 커널이 하는 일은 크게 어떻게 만들 것인가와 누구에게 CPU를 줄 것인가로 나뉩니다. 프로세스를 만들 때, 커널은 ID를 할당하고, 메모리 주소 공간 및 열린 파일 목록을 설정하고, 코드와 데이터를 할당된 메모리 안에 복사해 놓고, 실행 대기열에 추가합니다. 
