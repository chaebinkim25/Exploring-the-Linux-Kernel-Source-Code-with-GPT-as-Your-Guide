...
덕분에 개발자는 복잡한 물리적 현실은 잊고, 오직 코드의 논리에만 집중할 수 있는 것이죠.
...
실행 중인 프로그램을 우리는 `프로세스`라고 부릅니다. 커널에게 있어서 프로세스는 실행 중인 프로그램을 관리하기 위해서 ID, 상태, 할당받은 메모리, 열려 있는 파일 목록, CPU 쓸 때 우선 순위, 부모-자식 관계 등의 정보를 모아놓은 것입니다. `프로세스 관리`를 하는데 있어서 커널이 하는 일은 크게 어떻게 만들 것인가와 누구에게 CPU를 줄 것인가로 나뉩니다. 프로세스를 만들 때, 커널은 ID를 할당하고, 메모리 주소 공간 및 열린 파일 목록을 설정하고, 코드와 데이터를 할당된 메모리 안에 복사해 놓고, 실행 대기열에 추가합니다. 

CPU와 메모리를 제외한 모든 전자 모듈 (모니터, 키보드, 마우스 등등)을 `장치`라고 부릅니다. 커널에 있어서의 장치는 실제 장치를 설명하는 정보들입니다. 그 정보들 중에는 드라이버 (driver)라고 불리는 사용 설명서가 있어서 커널이 `장치 제어`를 할 수 있게 해줍니다. 커널은 드라이버에게 장치에서 읽고 쓰기를 하거나 설정을 바꾸라고만 하고, 실제로 장치와 통신을 하는 것은 드라이버가 합니다. 드라이버가 장치와 통신을 할 때, 장치에 접근할 권한은 커널에게만 있기 때문에, 커널에게 요청에서 가상 메모리 주소를 받아서 장치에 데이터를 읽고 쓸 수 있습니다. 한편, 키보드 입력 등 장치에서 어떤 일이 발생했다고 CPU에 직접 전기 신호로 알릴 수 있습니다. 드라이버는 CPU에 알림이 왔을 때 실행할 코드를 등록해두고, 커널은 알림이 오면 등록된 코드를 실행합니다.

`네트워킹`은 컴퓨터끼리 통신하는 것을 의미합니다. 리눅스 커널에서의 네트워킹은, 유저의 요청을 받아서 최종적으로 랜카드 드라이버를 통해 데이터를 송수신하는 것입니다. 랜카드에서 데이터가 수신되면 일단 데이터를 메모리에 복사해놓고, CPU에게 데이터가 도착했다고 알립니다. 커널은 데이터에서 수신 주소가 맞는지를 확인하고, 다른 곳으로 전달해야 하는지 분석합니다. 최종 도착지가 맞으면 어떤 프로그램이 기다리고 있는지를 확인하고, 적절한 프로그램에게 넘겨줍니다. 
